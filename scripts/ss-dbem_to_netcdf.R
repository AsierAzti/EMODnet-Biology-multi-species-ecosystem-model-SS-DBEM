...
var.def.nc(
  nc,
  varname    = "Biomass_relative",
  vartype    = "NC_DOUBLE",
  dimensions = c("lon", "lat", "aphiaid", "time")
)

att.put.nc(nc, "Biomass_relative", "_FillValue", "NC_DOUBLE", -99999)
att.put.nc(nc, "Biomass_relative", "long_name",  "NC_CHAR",   "Biomass relative change")
...
biomass_array <- array(
  ...
  dim = c(
    length(lon),
    length(lat),
    length(aphiaid_sorted),
    length(2030:2100)
  )
)

#Fill array
idx <- cbind(df$lon_i, df$lat_i, df$s_i, df$t_i)
